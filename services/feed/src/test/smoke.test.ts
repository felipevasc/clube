...
describe("feed smoke", () => {
  it("health + list", async () => {
    ...
    expect(res.body?.service).toBe("feed");

    // Only assert route registration here; calling it would require DB and a real HTTP server.
    const feedHandler = findRouteHandler(app as any, "get", "/feed");
    expect(typeof feedHandler).toBe("function");

    const reactHandler = findRouteHandler(app as any, "post", "/posts/:id/react");
    expect(typeof reactHandler).toBe("function");
  });
});
